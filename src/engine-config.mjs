...
export function makeEngineConfig(policy,auth){
 ...
 return {
  ...
  fiat_display_currency:'',timeframe:policy.timeframe??'15m',cancel_open_orders_on_exit:true,
  initial_state:'running',force_entry_enable:true,position_adjustment_enable:false,
  minimal_roi:demo?{}:{'0':0.03,'120':0.015,'360':0.005},stoploss:-0.02,
  ...(demo?{trailing_stop:false,use_custom_stoploss:true,amount_reserve_percent:0.05}:{}),
  ...
  order_types:{entry:'market',exit:'market',emergency_exit:'market',force_entry:'market',force_exit:'market',
   stoploss:demo&&!futures?'limit':'market',stoploss_on_exchange:demo,
   ...(demo?{stoploss_on_exchange_interval:15,...(futures?{stoploss_price_type:'last'}:{stoploss_on_exchange_limit_ratio:0.995})}:{})},
