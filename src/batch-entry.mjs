...
export async function waitForNativeEntryProtection({account,client,policy,local,pair,signal,
 ...
 for(;;){
  ...
  try{
   ...
   if(error.message!=='NATIVE_PROTECTION_POSITION_UNPROTECTED')throw error;
  }
  if(now()>=deadline)throw Error('NATIVE_ENTRY_PROTECTION_TIMEOUT');
...
export async function runEntryBatch({reference,snapshot,policy,client,local,modelEvidence,strategyVersion,executeFn,signal,
 ...
 try{
  for(let n=0;n<policy.pairs.length;n++){
   ...
   current=select(snapshot,policy,account,modelEvidence,{excludedPairs:[...attempted]});
  }
