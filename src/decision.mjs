...
import { orderFlowRuleDecision,demoRiskStake,DEMO_RULE_VERSION,FLOW_ONLY_PARAMETERS as DEMO_PARAMETERS,DEMO_PARAMETERS as AI_DEMO_PARAMETERS } from './demo-rules.mjs';
...
export function rulesProposal(snapshot,policy,account,modelEvidence,{excludedPairs=[],recentHistory=[],now=Date.now()}={}){
 const live=policy.mode!=='dry-run',ruleVersion=live?DEMO_RULE_VERSION:BASELINE_VERSION;
 const aiEntryEnabled=live&&snapshot?.aiAssist?.entryMode==='ai-only';
 const used=account.trades.reduce((s,t)=>s+Number(t.stake_amount),0),available=Math.max(0,Number(policy.maxExposureUsdt)-used);
 const lossCooldowns=live?recentLossCooldowns(recentHistory,{now}):new Map();
 const candidates=snapshot.markets.map(m=>{
  ...
  const cooldown=lossCooldowns.get(m.pair);
  if(cooldown&&rule.action!=='hold')return {pair:m.pair,...rule,action:'hold',stakeUsdt:'0',reasons:[...(rule.reasons??[]),'FLOW_PAIR_LOSS_COOLDOWN'],entryCooldown:{version:FLOW_LOSS_COOLDOWN_VERSION,durationMs:FLOW_LOSS_COOLDOWN_MS,...cooldown}};
 ...
 eligible.sort((a,b)=>(live&&!aiEntryEnabled?(Number(b.flowDiagnostics?.eligible===true)-Number(a.flowDiagnostics?.eligible===true)):0)||(live&&aiEntryEnabled?Number(b.selectionScoreBps)-Number(a.selectionScoreBps):policy.mode==='demo'?Number(b.flowStrength.delta)-Number(a.flowStrength.delta):0)||(live?Number(b.selectionScoreBps)-Number(a.selectionScoreBps):(b.targetFraction*10000-Number(b.requiredPriceSpaceBps))-(a.targetFraction*10000-Number(a.requiredPriceSpaceBps)))||a.pair.localeCompare(b.pair));
 ...
 const proposal={snapshotId:snapshot.id,action:entry?selected.action:'hold',pair,stakeUsdt:entry?selected.stakeUsdt:'0',
  ...
  reason:entry?'策略 '+ruleVersion+'：'+(live?(aiEntryEnabled?'固定模型的三步預測明確同意方向即可進場；成本、ATR 幾何、風險與原生保護仍必須通過，候選依模型剩餘空間排序，不代表預期獲利。':'訂單流成交力道與連續深度支持方向，且方向性中間價至少'+FLOW_SELECTIVITY.minimumMidChangeBps+' bps、五檔深度偏斜不超過'+(Number(FLOW_SELECTIVITY.maximumDepthImbalance)*100).toFixed(0)+'%；3 倍 ATR 目標扣除成本後至少覆蓋含成本停損風險，現貨優先依買盤深度優勢變化排序，再依計畫報酬與風險差排序，不是預期獲利。'):'已收 K 突破。')+(selected.adaptiveParameters?'每分鐘評估；本筆依價差、成本與深度調整風險預算至 '+Number(selected.adaptiveParameters.riskBudgetUsdt).toFixed(4)+' USDT，成交力道門檻 '+(Number(selected.adaptiveParameters.minTakerShare)*100).toFixed(2)+'%。':'依 1 USDT 預估風險預算縮倉。')+'使用已收盤 15 分鐘 ATR 的停損／停利；最多持倉 4 小時。實際績效以 Demo 成交紀錄為準。':
   '策略 '+ruleVersion+'：'+(selected&&full?'持倉或曝險容量不足。':aiEntryEnabled?(modelEvidence?.status==='not_due'?'等待下一根完整五分鐘 K 線與新 AI 預測；本分鐘只更新行情、倉位與風控。':'AI 預測未明確同意、成本／風險／價格空間未通過或模型證據尚未可用。'):'訂單流資料、方向支持或成本後報酬／風險未通過。')+' 詳細排除原因保存在 rules.json；持倉由引擎管理退出。',...(futures?{leverage:1}:{})};
 const aiAssist=live&&snapshot?.aiAssist?.enabled===true;
 const modelInvoked=aiAssist&&modelEvidence?.status!=='not_due';
 return {proposal,candidates,selected:entry?selected:null,metadata:{decisionEngine:'rules',llmInvoked:false,ruleVersion,parameters:live?(aiEntryEnabled?AI_DEMO_PARAMETERS:snapshot.volumeExperiment?{...DEMO_PARAMETERS,relativeVolumeMinimum:volumeMinimum(snapshot)}:DEMO_PARAMETERS):null,
  ...(live?{lossCooldown:{version:FLOW_LOSS_COOLDOWN_VERSION,durationMs:FLOW_LOSS_COOLDOWN_MS,blockedPairs:[...lossCooldowns.values()].map(v=>({pair:v.pair,closedAt:v.closedAt,ageMs:v.ageMs,profitAbs:v.profitAbs,tradeId:v.tradeId}))}}:{}),
  ...(live?{entrySignalEngine:aiEntryEnabled?'kronos_ai':aiAssist?'sampled_order_flow+kronos_advisory':'sampled_order_flow',modelInvoked,modelUsedForDecision:Boolean(selected?.model??selected?.aiAssist),modelStatus:modelEvidence?.status??'unavailable',modelReason:modelEvidence?.reason??null,modelFingerprint:modelEvidence?.modelFingerprint??null,predictionSha256:modelEvidence?.predictionSha256??null,volumeExperiment:snapshot.volumeExperiment??null,strategyContract:demoStrategyContract(policy,snapshot)}:{}),snapshotId:snapshot.id,timeframe:snapshot.timeframe??'15m',performanceSource:'demo-exchange-fills'}};
}
