...
export function rulesProposal(snapshot,policy,account,modelEvidence,{excludedPairs=[]}={}){
 ...
 const proposal={snapshotId:snapshot.id,action:entry?selected.action:'hold',pair,stakeUsdt:entry?selected.stakeUsdt:'0',
  ...
  reason:entry?'策略 '+ruleVersion+'：'+(live?'訂單流成交力道與連續深度支持方向；3 倍 ATR 目標扣除成本後至少覆蓋含成本停損風險，現貨優先依買盤深度優勢變化排序，再依計畫報酬與風險差排序，不是預期獲利。':'已收 K 突破。')+(selected.adaptiveParameters?'每分鐘評估；本筆依價差、成本與深度調整風險預算至 '+Number(selected.adaptiveParameters.riskBudgetUsdt).toFixed(4)+' USDT，成交力道門檻 '+(Number(selected.adaptiveParameters.minTakerShare)*100).toFixed(2)+'%。':'依 1 USDT 預估風險預算縮倉。')+'使用已收盤 15 分鐘 ATR 的停損／停利；最多持倉 4 小時。實際績效以 Demo 成交紀錄為準。':
