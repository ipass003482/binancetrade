...
export function technicalSummary(candles,timeframe='15m'){
 const spec=timeframeSpec(timeframe);
 if(!Array.isArray(candles)||candles.length<Math.max(20,spec.fourHourBars+1))throw new Error('INSUFFICIENT_TECHNICAL_DATA');
 const values=candles.map((c,i)=>{
  ...
  if(![o,h,l,close,v].every(x=>x.isFinite())||[o,h,l,close].some(x=>x.lte(0))||v.lt(0)
    ...
    ||c.openTime%spec.ms!==0||c.closeTime!==c.openTime+spec.ms-1
    ||(i&&c.openTime-candles[i-1].openTime!==spec.ms))throw new Error('INVALID_CANDLE_SERIES');
 ...
 return {timeframe,closedCandles:values.length,asOf:candles.at(-1).closeTime,
  lastClose:last.toFixed(),sma8:mean(closes.slice(-8)).toFixed(),sma20:mean(closes.slice(-20)).toFixed(),
  return1hPct:last.div(closes.at(-1-spec.hourBars)).minus(1).mul(100).toFixed(4),
  return4hPct:last.div(closes.at(-1-spec.fourHourBars)).minus(1).mul(100).toFixed(4),
