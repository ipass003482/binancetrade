...
import { loadEvaluationThresholds,performanceMarkdown } from './evaluation.mjs';
...
// Freqtrade fee_*_cost is QUOTE cost, even when fee_*_currency says BNB/base.
export function feeQuoteCurrency(t){
 const pairQuote=typeof t.pair==='string'?t.pair.split('/')[1]?.split(':')[0]:null;
 const quote=t.quote_currency??pairQuote;
 return typeof quote==='string'&&/^[A-Z0-9]+$/.test(quote)&&(!pairQuote||quote===pairQuote)?quote:null;
}
export function summarizeTrades(trades){
 ...
 for(const t of trades)for(const side of ['open','close']){
  ...
  const value=money(t['fee_'+side+'_cost']),currency=feeQuoteCurrency(t);
  if(value===null||value.isNegative()||!currency){missingFees.push({tradeId:t.trade_id,side});continue;}
 ...
 return {closedTrades:closed.length,openTrades:trades.filter(t=>t.is_open===true).length,
  ...
  closedTradeDrawdownUsdt:complete?drawdown.toFixed():null,engineFeesByQuoteCurrency:fees,missingFees,
  originalCommissionsByCurrency:null,
  notes:['PnL is reported by Freqtrade after its fee accounting; do not subtract fees again.',
   ...
   'Engine fee costs are quote-denominated estimates/accounting values, not quantities of the paid asset.',
   'Original exchange commission amounts are unavailable in this report; paid-asset labels must not be used as cost units.']};
}
export function decisionRows(proposals,records,trades){
 const latest=new Map(records.map(r=>[r.id,r]));
 return proposals.map(p=>{
  const pending=records.find(r=>r.status==='pending'&&r.snapshotId===p.snapshotId&&r.pair===p.pair);
  const filtered=records.find(r=>r.decisionStatus==='filtered'&&r.snapshotId===p.snapshotId&&r.pair===p.pair);
  const id=recordedEntryId(pending??filtered??p),entry=latest.get(id);
...
export function markdown(report){
 ...
 return ['# Binance trade 交易與決策報表','',
  ...
  s?'引擎費用（報價幣）：'+cell(JSON.stringify(s.engineFeesByQuoteCurrency))+'；缺少費用欄位：'+s.missingFees.length:'',
  '此處不是原始佣金明細；費用支付資產名稱不代表 fee_cost 的計量單位。',
  '損益已包含引擎的費用計算，不再重複扣費。此回撤不是帳戶權益回撤。','',
  '## 策略績效驗收','',performanceMarkdown(report.performance),'',
  '## 決策追蹤','',
  '| 動作 | 商品 | 送出狀態 | 交易 ID | 理由 |','|---|---|---|---|---|',
...
export async function buildReport(local,client,mode){
 ...
 try{
  ...
  const strategyVersion=await captureStrategyVersion({policy:client.policy??await loadPolicy(mode),engine:account.engine});
  history={mode,observedAt:new Date().toISOString(),trades,openTrades:account.trades,strategyVersion};
 ...
 if(history){
  const attribution=await readTradeVersions(local,trades,records,mode);
  const current=await captureStrategyVersion({policy:client.policy??await loadPolicy(mode),engine:history.strategyVersion?.engine});
  performance=evaluatePerformance({trades,mode,observedAt:history.observedAt,tradeVersions:attribution.versions,
   currentVersion:current.fingerprint,thresholds:await loadEvaluationThresholds()});
  for(const warning of attribution.warnings)warnings.push('策略版本來源無法核對：'+warning.tradeId);
 }
 ...
 const report={mode,generatedAt:new Date().toISOString(),historyFresh,observedAt:history?.observedAt??null,warnings,
  equity:await exists(join(local,'equity-summary.json'))?await readJson(join(local,'equity-summary.json')):null,
  summary:history?summarizeTrades(trades):null,performance,decisions:decisionRows(proposals,records,trades),
 ...
 return {mode,json:stem+'.json',markdown:stem+'.md',summary:report.summary,assessment:performance?.assessment??null,historyFresh,warnings};
}
