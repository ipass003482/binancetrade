...
export async function engineArguments(mode='dry-run') {
 ...
 if(config.dry_run!==!demo||config.trading_mode!=='spot'||config.exchange?.name!=='binance'
  ||config.exchange?.key||config.exchange?.secret||config.bot_name!==p.freqtrade.botName
  ||(demo && (config.exchange.demo_trading!==true||config.strategy!=='CodexDemoSpot'))
 ...
 if(demo)return [join(ROOT,'scripts/demo-engine.py')];
