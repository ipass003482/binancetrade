...
export async function execute({proposal,snapshot,policy,client,local,now=()=>Date.now(),getQuote=market}) {
 return lock(join(local,'execution.lock'),async()=>{
  ...
  const executionQuote=proposal.action==='buy'?await getQuote(proposal.pair,{mode:policy.mode}):null;
  ...
  await journalAppend(journal,{id,at,status:'pending',action:proposal.action,pair:proposal.pair,stakeUsdt:proposal.stakeUsdt,
    snapshotId:snapshot.id,tag,tradeId:decision.tradeId??null});
  try {
   if(proposal.action==='buy' && await exists(join(local,'STOP'))) {
   ...
   if(proposal.action==='buy' && (!Number.isInteger(result?.trade_id)||result.pair!==proposal.pair||result.enter_tag!==tag))
    throw new Error('UNEXPECTED_ORDER_RESPONSE');
   if(proposal.action==='sell' && result?.result!==('Created exit order for trade '+decision.tradeId+'.'))
