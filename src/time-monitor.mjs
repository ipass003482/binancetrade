...
export async function timeMonitor(local,mode,{getClock=readExchangeClock,now=()=>Date.now(),state=pidState}={}){
 ...
 const result={mode,timeframe,candleMs:CANDLE_MS,observedAt:new Date(now()).toISOString(),clock,clockError,serverNow:reference,
  ...
  nextCandleCloseAt:reference===null?null:new Date((Math.floor(reference/CANDLE_MS)+1)*CANDLE_MS).toISOString(),
 ...
 if(!stopped&&running&&reference!==null&&mode!=='dry-run'){
  ...
  const localDeadline=['waiting_candle','waiting_decision'].includes(health.stage)&&scheduled>now()?scheduled:
   nextDecisionBoundary(now(),await lastDecisionClaim(local),mode)+closeBufferMs(mode);
