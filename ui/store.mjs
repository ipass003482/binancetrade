...
export class DashboardStore extends EventTarget {
 constructor({fetchImpl=fetch}={}) {super();this.fetch=(...args)=>fetchImpl(...args);this.state={mode:'dry-run',pair:'BTC/USDT',preview:false,loading:false,data:null,market:null,error:null,marketError:null};this.version=0;}
 ...
 async refresh(update={}) {
  ...
  if(this.state.preview){this.publish({...previewData(this.state.mode,this.state.pair),loading:false});return;}
